...
export class SystemCollection {
  @Meta(SchemaType, 'system.collection.len') @Meta(Return, NS_SYSTEM_INT)
  static len(@Meta(SchemaType, 'system.array') array: unknown[]): number { return array.length; }

  @Meta(SchemaType, 'system.collection.contains') @Meta(Return, NS_SYSTEM_BOOL) @Meta(Generics, [{ name: 'T' }])
  static contains<T>(@Meta(SchemaType, 'system.array') array: T[], @Meta(SchemaType, 'T') value: T): boolean { return array.includes(value); }

  @Meta(SchemaType, 'system.collection.notcontains') @Meta(Return, NS_SYSTEM_BOOL) @Meta(Generics, [{ name: 'T' }])
