...
// language
const langWatches = new Observable<[string, {[key:string]: string}]>()
...
export function setLanguage(...languages: string[]) 
{
    ...
    langWatches.onNext(currLang, currLocale)
    return currLang
}
...
export function subscribeLanguage(func: Observer<[string, {[key:string]: string}]>, immediate?: boolean) : Function
{
    const handler = langWatches.subscribe(func)
    if (immediate && handler) handler(currLang, currLocale)
    return handler
}
